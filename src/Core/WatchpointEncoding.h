...
namespace hs
{
	...
	// MUTATION M2 (temporary): one window, forced through the post-free
	// direction -- the 0.6.3 conflation of the two allocator orderings.
	const auto delta = a_trapTick >= a_freeTick ? (a_trapTick - a_freeTick) : (a_freeTick - a_trapTick);
	return delta <= a_windowMs ? WatchpointFreeContext::kPostFreeLink : WatchpointFreeContext::kDelayedWriteAfterFree;
	}
}